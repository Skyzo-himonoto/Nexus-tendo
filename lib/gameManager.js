...
class GameManager {
  constructor() {
    ...
    this.gameHistory = new Map(); // menyimpan history game
    this.init();
  }
  
  async init() {
    await fs.ensureDir(this.gamesPath);
    await this.ensureGameFiles();
  }
  
  async ensureGameFiles() {
    const gameFiles = [
      'truth.json', 'dare.json', 'tebakkimia.json', 'tebaktebakan.json',
      'tebakmakanan.json', 'bucin.json', 'caklontong.json', 'family100.json',
      'tebakbendera2.json', 'tebakfilm.json', 'tebaklirik.json', 'tebaklagu.json',
      'tekateki.json', 'tebakprofesi.json', 'tebaknegara.json', 'siapakahaku.json',
      'susunkata.json', 'tebakkata.json', 'tebakkabupaten.json', 'tebakjkt48.json',
      'riddle.json', 'renungan.json', 'kataacak.json', 'tebakhewan.json', 'tebakepep.json'
    ];
  ...
  async loadGameData(gameName) {
    const filePath = path.join(this.gamesPath, `${gameName}.json`);
    return await loadJSON(filePath);
  }
  
  async saveGameData(gameName, data) {
    const filePath = path.join(this.gamesPath, `${gameName}.json`);
    return await saveJSON(filePath, data);
  }
  ...
  startGame(groupId, gameType, player, duration = 30) {
    if (this.activeGames.has(groupId)) {
      return { success: false, message: 'Masih ada game yang berlangsung di grup ini!' };
    }
  ...
  endGame(groupId) {
    ...
    if (session) {
      this.gameHistory.set(`${groupId}_${session.id}`, session);
  ...
  async checkAnswer(groupId, answer, player) {
    const session = this.activeGames.get(groupId);
    if (!session) {
      return { success: false, message: 'Tidak ada game yang sedang berlangsung!' };
    }
    
    if (session.player !== player) {
      return { success: false, message: 'Ini bukan game kamu!' };
    }
    
    const currentQuestion = session.currentQuestion;
    if (!currentQuestion) {
      return { success: false, message: 'Tidak ada pertanyaan aktif!' };
    }
    ...
    if (isCorrect) {
      ...
      return { 
        success: true, 
        correct: true, 
        message: `✅ Benar! +20 poin\nSkor: ${session.score + 1}`,
      ...
      return { 
        success: true, 
        correct: false, 
        message: `❌ Salah! Jawaban yang benar adalah: *${currentQuestion.answer}*`,
        score: session.score
      };
    }
  }
  ...
  async getGameStats(gameName) {
    ...
    return {
      ...
      lastUpdated: data[data.length - 1]?.createdAt || null
    };
  }
  ...
  async getAllGamesStats() {
    ...
    for (const game of games) {
      stats[game] = await this.getGameStats(game);
    }
    return stats;
  }
}
