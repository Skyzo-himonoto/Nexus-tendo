export default async function kick(context) {
  ...
  if (mentioned.length === 0 && args.length === 0) {
    return await sock.sendMessage(sender, { text: '📝 .kick @user atau .kick 628xxxxx' });
  }
  ...
  for (const target of targets) {
    try { await sock.groupParticipantsUpdate(sender, [target], 'remove'); success.push(target); } 
    catch { failed.push(target); }
  }
  
  let text = '';
  if (success.length) text += `✅ Berhasil mengeluarkan:\n${success.map(s => `@${s.split('@')[0]}`).join('\n')}\n`;
