...
export default async function tebakfilm(context) {
  const { sock, sender, args, m } = context;
  const groupId = sender;
  
  if (args.length === 0) return await sock.sendMessage(sender, { text: '🎬 TEBAK FILM\n\n📝 .tebakfilm start\n💰 Hadiah: 50 poin' });
  ...
  if (cmd === 'start') {
    ...
    try {
      const res = await axios.get('https://api.ryzendesu.vip/api/game/tebakfilm');
      const data = res.data;
