...
╰━━━━━━━━━━━━━━━━━━━╯`
  ...
  if (command === 'toimg') {
    ...
    if (!isVideo) return await sock.sendMessage(sender, { text: '📝 Reply video dengan .tomp3' });
    ...
    await sock.sendMessage(sender, { audio: { url: outputPath }, mimetype: 'audio/mpeg', fileName: `${randomString()}.mp3`, caption: '🎵 Audio extracted!' });
    await fs.unlink(inputPath);
    await fs.unlink(outputPath);
  }
    ...
    if (!isAudio) return await sock.sendMessage(sender, { text: '📝 Reply audio dengan .tomp4' });
    ...
    await new Promise((resolve, reject) => {
      ffmpeg().input('color=black:s=1280x720:d=30').input(inputPath)
        .outputOptions(['-c:v libx264', '-c:a aac', '-shortest'])
        .on('end', resolve).on('error', reject).save(outputPath);
    });
    
    await sock.sendMessage(sender, { video: { url: outputPath }, caption: '🎬 Video harus dari audio!' });
    await fs.unlink(inputPath);
    await fs.unlink(outputPath);
  }
    ...
    const isVideo = quoted?.videoMessage || m.message?.videoMessage;
    if (!isVideo) return await sock.sendMessage(sender, { text: '📝 Reply video dengan .togif' });
    ...
    await new Promise((resolve, reject) => {
      ffmpeg(inputPath).outputOptions(['-vf', 'fps=10,scale=320:-1']).toFormat('gif')
        .on('end', resolve).on('error', reject).save(outputPath);
    });
    
    await sock.sendMessage(sender, { image: { url: outputPath }, caption: '🎪 GIF created!', mimetype: 'image/gif' });
    await fs.unlink(inputPath);
    await fs.unlink(outputPath);
  }
    ...
    if (!isImage) return await sock.sendMessage(sender, { text: '📝 Reply gambar dengan .towebp' });
    ...
    await sock.sendMessage(sender, { text: `❌ Converter *${command}* tidak dikenal!\n\nGunakan .converter untuk melihat daftar.` });
  }
  }
